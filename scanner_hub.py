...
# Yahoo Finance predefined screener IDs (verified working)
_YF = 'https://query1.finance.yahoo.com/v1/finance/screener/predefined/saved?count=50&scrIds='
SCREENER_URLS = {
    'most_active':      _YF + 'most_actives',
    'day_gainers':      _YF + 'day_gainers',
    'day_losers':       _YF + 'day_losers',
    'trending':         _YF + 'most_actives',
    'week52_high':      _YF + '52_wk_gainers',
    'unusual_volume':   _YF + 'most_actives',   # Yahoo has no direct unusual vol screener
    'small_cap':        _YF + 'small_cap_gainers',
    'large_cap':        _YF + 'large_cap_gainers',
    'highest_beta':     _YF + 'most_actives',
    'highest_dividend': _YF + 'high_yield_bond',
    'most_expensive':   _YF + 'most_actives',
    'pink_sheet':       _YF + 'most_actives',
}

MAX_WORKERS   = 6    # parallel fetch threads — keep low to avoid Yahoo rate limits
...
# ── Yahoo crumb/cookie session (required for cloud IPs) ──────────────────────
_yahoo_crumb: str | None = None
_yahoo_crumb_ts: float   = 0.0
_crumb_lock = threading.Lock()

def get_yahoo_crumb() -> str | None:
    """Obtain a Yahoo Finance crumb cookie — required for cloud/server IPs."""
    global _yahoo_crumb, _yahoo_crumb_ts
    with _crumb_lock:
        if _yahoo_crumb and (time.time() - _yahoo_crumb_ts) < 3600:
            return _yahoo_crumb
        session = get_session()
        try:
            ...
            if r.ok and r.text and r.text != 'null':
                ...
                log.info(f'Yahoo crumb acquired: {_yahoo_crumb[:8]}...')
                return _yahoo_crumb
...
def yahoo_get(url: str, timeout: int = 10):
    """
    Fetch a Yahoo Finance URL with crumb auth, circuit-breaker and 429 handling.
    ...
    if r.status_code == 401 or (r.status_code == 403 and 'crumb' in url.lower()):
        # Crumb expired — refresh and retry once
        global _yahoo_crumb, _yahoo_crumb_ts
        _yahoo_crumb = None; _yahoo_crumb_ts = 0.0
        crumb = get_yahoo_crumb()
        if crumb:
            url = url.split('&crumb=')[0].split('?crumb=')[0]
            sep = '&' if '?' in url else '?'
            url = f'{url}{sep}crumb={crumb}'
            r = session.get(url, timeout=timeout)
...
def fetch_and_analyse(symbol: str, cfg: dict = None, retry: bool = True) -> dict | None:
    ...
    # 15m chart captures overnight/late extended hours candles that 1m misses
    intra_url2 = (f'https://query1.finance.yahoo.com/v8/finance/chart/{symbol}'
                  f'?interval=15m&range=5d&includePrePost=true')
    ...
    # ── Step 0: quoteSummary price module — most reliable for pre/post ──────────
    # Uses v10/quoteSummary which returns structured pre/post prices
    qs_url = (f'https://query1.finance.yahoo.com/v10/finance/quoteSummary/{symbol}'
              f'?modules=price')
    try:
        rq = yahoo_get(qs_url, timeout=8)
        price_mod = (rq.json().get('quoteSummary') or {}).get('result', [None])[0]
        if price_mod:
            pm = price_mod.get('price', {})
            def _raw(d): return d.get('raw') if isinstance(d, dict) else d
            state   = _raw(pm.get('marketState')) or state
            regular = _raw(pm.get('regularMarketPrice')) or regular
            pre     = _raw(pm.get('preMarketPrice'))
            post    = _raw(pm.get('postMarketPrice'))
            log.info(f'{symbol} quoteSummary: state={state} reg={regular} pre={pre} post={post}')
    except Exception as e:
        log.debug(f'{symbol} quoteSummary fallback: {e}')

    # Also try v7 quote as secondary source
    if not (pre or post):
        quote_url = (f'https://query1.finance.yahoo.com/v7/finance/quote'
                     f'?symbols={symbol}')
        try:
            rq = yahoo_get(quote_url, timeout=6)
            qr = rq.json().get('quoteResponse', {}).get('result', [None])[0]
            if qr:
                if not state:   state   = qr.get('marketState')
                if not regular: regular = qr.get('regularMarketPrice')
                if not pre:     pre     = qr.get('preMarketPrice')
                if not post:    post    = qr.get('postMarketPrice')
                log.debug(f'{symbol} v7: state={state} pre={pre} post={post}')
        except Exception as e:
            log.debug(f'{symbol} v7 fallback: {e}')

    # ── Step 1: intraday for freshest live price + last_tick ──────────────────
    try:
        r = yahoo_get(intra_url, timeout=8)
        res = r.json().get('chart', {}).get('result', [None])[0]
        if res:
            m = res.get('meta', {})
            if not state:   state   = m.get('marketState')
            if not regular: regular = m.get('regularMarketPrice')
            if not pre:     pre     = m.get('preMarketPrice')
            if not post:    post    = m.get('postMarketPrice')
            q1m  = res.get('indicators', {}).get('quote', [{}])[0]
            ts_list = res.get('timestamp') or []
            c1m  = [v for v in (q1m.get('close') or []) if v is not None]
            if c1m:
                last_tick = c1m[-1]
                # Determine session from last candle timestamp
                if ts_list:
                    import datetime, zoneinfo
                    try:
                        lt = datetime.datetime.fromtimestamp(ts_list[-1],
                             tz=zoneinfo.ZoneInfo('America/New_York'))
                        h, mn = lt.hour, lt.minute
                        total = h * 60 + mn
                        if 570 <= total < 960:   # 9:30am–4:00pm ET = REGULAR
                            state = state or 'REGULAR'
                        elif (240 <= total < 570) or total < 240:  # midnight–9:30am ET = PRE
                            state = 'PRE'
                            if not pre: pre = last_tick
                            log.info(f'{symbol} pre-mkt tick={last_tick:.2f} at {lt.strftime("%H:%M")} ET')
                        elif total >= 960:  # 4:00pm–midnight ET = POST (extended window)
                            state = 'POST'
                            if not post: post = last_tick
                            log.info(f'{symbol} after-hrs tick={last_tick:.2f} at {lt.strftime("%H:%M")} ET')
                    except Exception:
                        pass
    except Exception as e:
        log.warning(f'{symbol} intraday: {e}')

    # ── Step 1b: 15m chart — catches overnight candles after 8pm ET ──────────
    # Yahoo's 1m data stops storing extended hours after ~8pm ET
    # but 15m data keeps them. ALWAYS run this to find the freshest extended price.
    # Don't gate on post/pre — meta values can be stale.
    if True:  # always fetch 15m for most accurate extended hours price
        try:
            r15 = yahoo_get(intra_url2, timeout=8)
            res15 = r15.json().get('chart', {}).get('result', [None])[0]
            if res15:
                m15 = res15.get('meta', {})
                if not state:   state   = m15.get('marketState')
                if not regular: regular = m15.get('regularMarketPrice')
                if not pre:     pre     = m15.get('preMarketPrice')
                if not post:    post    = m15.get('postMarketPrice')
                # Parse candle timestamps to find latest extended hours candle
                ts15   = res15.get('timestamp') or []
                q15    = res15.get('indicators', {}).get('quote', [{}])[0]
                c15    = q15.get('close') or []
                if ts15 and c15:
                    import datetime, zoneinfo
                    et = zoneinfo.ZoneInfo('America/New_York')
                    # Find the latest valid candle
                    for i in range(len(ts15)-1, -1, -1):
                        if i < len(c15) and c15[i] is not None:
                            lt15 = datetime.datetime.fromtimestamp(ts15[i], tz=et)
                            total15 = lt15.hour * 60 + lt15.minute
                            candle_price = float(c15[i])
                            if 570 <= total15 < 960:
                                # Regular hours candle — use as fallback only
                                if not last_tick: last_tick = candle_price
                            elif total15 >= 960 or total15 < 570:
                                # Extended hours candle (after 4pm or before 9:30am ET)
                                # Always override — this is fresher than meta.postMarketPrice
                                post = candle_price
                                log.info(f'{symbol} 15m extended: {candle_price:.2f} at {lt15.strftime("%H:%M")} ET (total={total15})')
                            break
        except Exception as e:
            log.debug(f'{symbol} 15m fetch: {e}')
    ...
    if   s == 'REGULAR':
        ...
        if pre:    price, mode = pre,  'PRE-MKT'
            ...
            # Use last_tick regardless of whether it differs from regular
            # — it IS the most recent traded price (may be after-hours candle)
            diff_pct = abs(last_tick - regular) / regular * 100 if regular else 0
            price = last_tick
            mode  = 'AFTER-HRS' if diff_pct > 0.05 else 'CLOSE'
...
def fetch_all(pool: list, cfg: dict = None) -> list:
    ...
    def go(sym):
        time.sleep(random.uniform(0, 0.5))  # jitter up to 500ms to avoid 429s
