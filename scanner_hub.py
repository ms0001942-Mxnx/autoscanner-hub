...
def fetch_and_analyse(symbol: str, cfg: dict = None, retry: bool = True) -> dict | None:
    ...
    # but 15m data keeps them. Use this to find overnight prices.
                ...
                if ts15 and c15:
                    ...
                    for i in range(len(ts15)-1, -1, -1):
                        if i < len(c15) and c15[i] is not None:
                            ...
                            elif total15 >= 960 or total15 < 240:
                                # Extended hours — this IS the overnight price
                                if not post:
                                    post = candle_price
                                    log.info(f'{symbol} 15m overnight: {candle_price:.2f} at {lt15.strftime("%H:%M")} ET')
                            break
        except Exception as e:
