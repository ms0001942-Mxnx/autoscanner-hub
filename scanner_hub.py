...
def fetch_and_analyse(symbol: str, cfg: dict = None, retry: bool = True) -> dict | None:
    ...
    # ── Step 0: v7 quote endpoint — most reliable for pre/post prices ─────────
    quote_url = (f'https://query1.finance.yahoo.com/v7/finance/quote'
                 f'?symbols={symbol}&fields=regularMarketPrice,preMarketPrice,'
                 f'postMarketPrice,marketState,regularMarketPreviousClose')
    try:
        rq = yahoo_get(quote_url, timeout=6)
        qr = rq.json().get('quoteResponse', {}).get('result', [None])[0]
        if qr:
            state   = qr.get('marketState')
            regular = qr.get('regularMarketPrice')
            pre     = qr.get('preMarketPrice')
            post    = qr.get('postMarketPrice')
            log.debug(f'{symbol} v7 quote: state={state} regular={regular} pre={pre} post={post}')
    except Exception as e:
        log.debug(f'{symbol} v7 quote fallback: {e}')
